...
interface BasicDicomImageSectionProps {
...
const BasicDicomImageSectionComponent = ({ dicomPath, title, caseId, onMetadataLoaded }: BasicDicomImageSectionProps) => {
  ...
  const generateSignedUrl = useCallback(async (dicomPath: string) => {
    ...
    try {
      console.log("BasicDicomImageSection: Generating signed URL for:", dicomPath);
      ...
      console.error("BasicDicomImageSection: Failed to generate signed URL:", error);
  ...
  const handleViewerError = useCallback((error: Error) => {
    console.error("BasicDicomImageSection: Viewer error:", error);
          ...
          <div className="relative bg-gray-50 max-w-full overflow-auto rounded-lg border border-medical-border">
            {isLoadingScans ? (
              ...
              <BasicDicomViewer 
                ...
                className="bg-gray-50 min-h-[400px]"
...
export const BasicDicomImageSection = memo(BasicDicomImageSectionComponent);
